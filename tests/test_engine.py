...
from engine import gerar_comentario_contextual, gerar_conteudo_completo
...
def test_gerar_comentario_contextual_com_sucesso(mocker):
    ...
    mocker.patch("engine.genai.Client", return_value=mock_client)
    mocker.patch("engine.os.getenv", return_value="fake_api_key")
...
def test_gerar_comentario_contextual_erro_api_retorna_fallback(mocker):
    ...
    mocker.patch("engine.genai.Client", side_effect=Exception("Erro de Conexão"))
    mocker.patch("engine.os.getenv", return_value="fake_api_key")
