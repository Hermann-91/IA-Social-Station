...
def iniciar_tela_principal():
    ...
    # Inicializa o Facade único do Robô
    try:
        fachada = FachadaRobo()
    except Exception as e:
        st.error(f"Erro ao inicializar o motor de IA: {e}")
        st.info("Dica: Certifique-se de que a sua chave GEMINI_API_KEY está configurada no arquivo .env.")
        return

    # Inicializa estados de sessão do Streamlit
        ...
        with col_acoes:
            ...
            if btn_capturar:
                with st.spinner("Conectando ao Chrome e capturando estado..."):
                    try:
                        dados = fachada.capturar_postagem_chrome()
            ...
            if st.session_state.captura:
                ...
                if btn_gerar_comment:
                    with st.spinner("Gemini gerando comentário contextualizado (Análise Multimodal ativa)..."):
                        try:
                            comment = fachada.gerar_comentario_contextual(texto_sel)
                ...
                if st.session_state.comentario_gerado:
                    ...
                    if btn_postar:
                        with st.spinner("Enviando comentário e submetendo no Chrome..."):
                            try:
                                msg = fachada.postar_comentario_chrome(comentario_revisado)
    ...
    with aba_criar_post:
        ...
        fila = fachada.obter_postagens_fila()
        ...
        if btn_gerar_lote:
            ...
            for i, config in enumerate(configs_lote):
                ...
                with st.spinner(f"Processando Post {i+1} no Gemini..."):
                    try:
                        post_gerado = fachada.planejar_e_adicionar_postagem(caminhos_locais, config["prompt"])
                        posts_novos.append(post_gerado)
                    except Exception as e:
        ...
        if not fila:
            ...
            for idx, post in enumerate(fila):
                agendado_info = f" | 📅 Agendado para {post.data_hora_agendamento}" if post.data_hora_agendamento and post.status == 'Agendado' else ""
                with st.expander(f"Post {idx+1} - Horário sugerido: {post.horario_sugerido} ({post.status}{agendado_info})"):
                    ...
                    with col_prev:
                        if post.caminhos_midias and os.path.exists(post.caminhos_midias[0]):
                            ext = post.caminhos_midias[0].split(".")[-1].lower()
                            if ext in ["png", "jpg", "jpeg"]:
                                st.image(post.caminhos_midias[0], use_container_width=True)
                            else:
                    ...
                    with col_detalhes:
                        ...
                        if legenda_ed != post.legenda:
                            post.legenda = legenda_ed
                            fachada.repositorio.salvar(post)
                        ...
                        with col_agendar:
                            ...
                            if btn_agendar:
                                ...
                                dt_str = dt_combinado.strftime("%Y-%m-%d %H:%M:%S")
                                fachada.agendar_postagem(post.id, dt_str)
                        ...
                        if btn_postar_fila:
                            with st.spinner("Automação ativa... Publicando no Instagram..."):
                                try:
                                    res_pub = fachada.publicar_postagem_agora(post.id)
                        ...
                        if btn_remover_fila:
                            fachada.remover_postagem(post.id)
        ...
        if btn_gerar_hash:
            if not tema_hash:
                ...
                with st.spinner(f"Analisando métricas de engajamento para '{tema_hash}'..."):
                    try:
                        resultado_hash = fachada.pesquisar_hashtags(tema_hash, qtd_hash)
